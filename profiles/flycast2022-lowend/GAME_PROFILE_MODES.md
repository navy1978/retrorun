...
I profili validati su RG351MP, RG353M e RG552 sono condivisi rispettivamente
dalle famiglie RK3326, RK3566 e RK3399; il nome del dispositivo resta indicato
come provenienza della validazione.
...
| Soul Calibur | `T1401D  50`, `T1401M`, `T1401N` | `retrorun-soul-T1401D50.cfg` | `experimental/retrorun-soul-performance-T1401D50.cfg` | Le edizioni retail europea, giapponese e nordamericana usano lo stesso profilo generico. Su RK3566 entrambe usano `framerate=normal` e disabilitano il pacing del frontend; la variante veloce usa `top_hud_last` e può mostrare il fondale sopra la barra. RK3399 ha invece il profilo completo `best_validated` approvato su RG552: frontend video multithread, audio accurato senza WSOLA e `render_queue_no_drop`; `best_performance` ricade sullo stesso profilo. |
...
| Shenmue II | `MK-5118450`, `HDR-0164`, `HDR-0179` | Il profilo europeo usa rendering conservativo e audio `lowend_heavy_100`; le varianti giapponesi restano baseline | Su RG351MP il profilo europeo aggiunge il batch AICA accurato; RG353M usa il proprio profilo validato | Il batch AICA conserva il mixer accurato e sposta controlli stabili fuori dal ciclo interno di 32 campioni. Nel confronto identico da tre run/600 frame ha guadagnato +1,44% senza cambiare la mediana di cinque underrun. È abilitato soltanto sul profilo RG351MP verificato e non si applica alle varianti giapponesi non testate. |
| Sega Rally 2 | `MK-51019`, `HDR-0010` | Baseline globale; profili RK3326, RK3566 e RK3399 | Su RK3326 `best_performance` usa il fallback sicuro `best_validated`; RK3566 usa il profilo prestazioni validato su RG353M; RK3399 ricade sul profilo RG552 validato | Il profilo RK3326 conserva tutto l'environment e AICA 32. Nel test finale su RG351MP è salito da 13,43/7,13 a 15,74/11,80 core/presented FPS, ma l'audio e il limite RK3326 restano documentati. |
| Street Fighter III: 3rd Strike | `T7013D50`, `T1213N`, `T1209M` | alpha `per-triangle`, merge traslucido e riuso texture off, fast depth `vertex_fast_log`, merge opaco attivo e audio accurato | Su RK3326 seleziona il core `upstream_620` | Il profilo USA è stato validato manualmente su RG351MP/dArkOS da un savestate di combattimento e poi associato alle tre varianti retail regionali con impostazioni identiche. Il profilo grafico originale raggiungeva 47,76 FPS contro 44,39 (+7,58%). Il successivo confronto a tre run ha misurato 58,05 FPS con `upstream_620` contro 46,73 con il core corrente (+24,2%); personaggi, fondali, animazioni, controlli e audio sono stati approvati manualmente. Lo stesso profilo è ora condiviso dall'intera famiglia RK3326. |
| Sonic Adventure | `MK-51000` | Baseline sicura; su RK3566 eredita il rendering globale approvato | Profilo globale; profilo RK3566 con pacing frontend disattivato e audio 735 frame/60 ms senza buffer stabile | Sul savestate RG353M ha misurato 27,96 FPS contro 25,59 dello stack dArkOS stock (+9,3%), p95 64,11 contro 69,22 ms e nessun frame saltato, underrun o coda vuota. Video, audio e gameplay approvati manualmente. Le varianti giapponesi `HDR-0001` e `HDR-0043` restano baseline. |
| Power Stone 2 | `T36812D61`, `T36812D64`, `T1218M`, `T1211N` | Baseline conservativa | Su RK3566: alpha accurato, `menu_guarded`, riuso texture, fast depth protetto, merge opaco, mixer rapido/AICA 16 e audio 735 frame/60 ms | Sul combattimento USA testato su RG353M ha raggiunto 58,96 FPS contro 56,17 stock (+5,0%), senza frame saltati o errori audio. Il sorter veloce valeva appena 0,13 FPS in più; fast depth, AICA 8 e ulteriore riuso stati erano più lenti. Video, HUD, audio e gameplay approvati manualmente; le varianti retail condividono il profilo. |
...
| Shenmue II (Europe, RG351MP) | prestazioni | on/on | merge off, alpha `per-strip` | `vertex_fast_log` | `lowend_heavy_100`, mixer e batch `accurate` | off | D24S0 |
| Sega Rally 2 (RG351MP) | validata | on/on | merge off, alpha `per-triangle` | off | `lowend_heavy_100`, mixer accurato, AICA 32 | off | D24S0 |
...
3. Il profilo effettivo viene composto nell'ordine globale, chip e infine
   override specifico del dispositivo. Ogni livello sovrascrive soltanto le
   opzioni che dichiara.
4. I profili con impostazioni esplicite conservano i default low-end usati
...
5. Un gioco non testato, sconosciuto o privo di Product number non riceve
   alcuna modifica: continuano a valere tutti i valori del `retrorun.cfg`.
6. RetroRun usa il catalogo incorporato, oppure
