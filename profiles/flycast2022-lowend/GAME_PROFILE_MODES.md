...
| Soul Calibur | `T1401D  50`, `T1401M`, `T1401N` | `retrorun-soul-T1401D50.cfg` | `experimental/retrorun-soul-performance-T1401D50.cfg` | Le edizioni retail europea, giapponese e nordamericana usano lo stesso profilo generico. Su RG353M entrambe usano `framerate=normal` e disabilitano il pacing del frontend; la variante veloce usa `top_hud_last` e può mostrare il fondale sopra la barra. RG552 ha invece un profilo completo `best_validated` specifico: configurazione stock-equivalent approvata, frontend video multithread, audio accurato senza WSOLA e `render_queue_no_drop`; `best_performance` ricade sullo stesso profilo. La policy della coda non è abilitata altrove. |
...
| Sega Rally 2 | `MK-51019`, `HDR-0010` | Baseline globale; override specifici per RG351MP e RG353M | Su RG351MP `best_performance` usa il fallback sicuro `best_validated`; RG353M usa il profilo prestazioni validato | Il profilo RG351MP conserva tutto l'environment e AICA 32. Nel test finale è salito da 13,43/7,13 a 15,74/11,80 core/presented FPS, ma l'audio e il limite RK3326 restano documentati. La variante giapponese non è stata promossa su RG351MP. |
| Street Fighter III: 3rd Strike | `T7013D50`, `T1213N`, `T1209M` | alpha `per-triangle`, merge traslucido e riuso texture off, fast depth `vertex_fast_log`, merge opaco attivo e audio accurato | Su RG351MP seleziona il core `upstream_620`; sugli altri RG351 eredita il rispettivo `best_validated` | Il profilo USA è stato validato manualmente su RG351MP/dArkOS da un savestate di combattimento e poi associato alle tre varianti retail regionali con impostazioni identiche. Il profilo grafico originale raggiungeva 47,76 FPS contro 44,39 (+7,58%). Il successivo confronto a tre run ha misurato 58,05 FPS con `upstream_620` contro 46,73 con il core corrente (+24,2%); personaggi, fondali, animazioni, controlli e audio sono stati approvati manualmente. Gli altri RG351 mantengono il core corrente finché non vengono testati. |
